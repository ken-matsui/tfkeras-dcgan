...
class Trainer(object):
	...
	def fit(self, X, epochs=10, batch_size=1000, plotting=True):
		...
		:return:
		"""
		data = mnist.read_data_sets("./", one_hot=True)
		...
		x_data = tf.placeholder(tf.float32, shape=(batch_size, 28, 28, 1))
		...
		with tf.Session() as sess:
			sess.run(tf.group(init, tf.local_variables_initializer()))
			for epoch in range(epochs):
				...
				while True:
					try:
						...
						batch_xs = tf.reshape(batch_xs, [100, 28, 28, 1])
						z_data = np.random.uniform(-1, 1, (batch_size, self.z_dim))
						train_fd = { z: z_data, x_data: batch_xs, K.learning_phase(): 1 }
						_, gen_loss_val, y_val = sess.run([gen_train_step, gen_loss, y_pred1], feed_dict=train_fd)
